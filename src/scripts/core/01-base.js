  ...
  /* Значок (емодзі) для папки, цілі, виноски. Його часто пише модель, а
     рядок з моделі може бути отруєний: замість «📁» прийде HTML з кодом,
     який потім вставиться в сторінку. Значок — це 1–2 символи-картинки
     (з ZWJ-склейками й прапорцями), тож усе довше за 16 знаків або з
     «HTML-символами» відкидаємо і ставимо запасний значок. */
  function safeEmoji(s, def){
    const t = String(s==null?'':s).trim();
    if(t && t.length<=16 && !/[<>&"'`=\\/\s]/.test(t)) return t;
    return def==null ? '' : def;
  }
